...
Week 2 showed that the model should be treated as a proposal generator. Week 3 reinforced that boundary after fine-tuning improved controlled-task accuracy but still failed independent privilege-escalation probes. Typed validation accepts or rejects the proposal, application policy owns the final route, and the versioned deterministic catalogue owns authoritative ATT&CK candidates. The current retrieval tools are implemented independently; automated route-triggered RAG synthesis remains target architecture, not a completed capability.
